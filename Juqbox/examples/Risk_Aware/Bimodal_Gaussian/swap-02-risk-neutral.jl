...
# Default values for uniform distribution on [-ϵ/2,ϵ/2]^4
# assume there are 4 independent, uniform noise along Diagonal
if(!@isdefined(ep_max))
    ep_max = 2*pi*3e-2
end
...
#       E[J] = ∑ w[k] J[x[k]]
# where w,x are the weights and nodes on [-ϵ/2,ϵ/2]^4
...
if(!@isdefined(dim_noise))
    dim_noise = 4
end

function monteCarlo1(nquad::Int64)
    weights = ones(nquad).*(1/nquad) # each weight is 1/N
    nodes = (rand(Float64, (dim_noise,nquad)).- 0.5) .* ep_max # Map nodes to [-ϵ/2,ϵ/2]^4
...
# Random.seed!()
# nquad = 100
# nodes, weights = monteCarlo1(nquad)
# writedlm("nodes_100.dat", nodes)
# writedlm("weights.dat", weights)

#### fix MC samples
if nquad == 1
    nodes = zeros(dim_noise, 1)
    weights = ones(1)
else
    nodes = readdlm("nodes_100.dat")
    weights = ones(nquad).*(1/nquad)
end
...
params = Juqbox.objparams([N], [Nguard], T, nsteps, Uinit=U0, Utarget=vtarget, Cfreq=om, Rfreq=rot_freq,
                          Hconst=H0, Hsym_ops=Hsym_ops, Hanti_ops=Hanti_ops, wmatScale=0.01)
...
prob = Juqbox.setup_ipopt_problem(params, wa, nCoeff, minCoeff, maxCoeff, maxIter=maxIter, lbfgsMax=lbfgsMax, 
                                  startFromScratch=startFromScratch, ipTol=ipTol,acceptTol=acceptTol, acceptIter=acceptIter, 
                                  nodes=nodes, weights=weights, switch_loss=switch_loss, hyper=hyper)

println("Initial coefficient vector stored in 'pcof0'")
println("using ", nquad, " quadrature nodes")
println("loss function = ", switch_loss, " (1:RN; 2:RS; 3:RA), with hyper = ", hyper)
