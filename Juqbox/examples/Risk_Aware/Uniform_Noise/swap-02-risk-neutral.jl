...
# MC sampling
weights = ones(nquad).*(1/nquad) # each weight is 1/N
nodes = (rand(Float64, nquad).- 0.5) .* ep_max # Map nodes to [-ϵ/2,ϵ/2]


# change loss functions, use different random seeds
if(!@isdefined(switch_loss))
    switch_loss = 1
end
...
# whether using cvar loss
if switch_loss == 4
    use_cvar = true
    println("using CVaR loss, with beta = ", hyper)
else
    use_cvar = false
end

Random.seed!(2222)
# this time try not fix random seeds
# if switch_loss == 1
#     Random.seed!(2222)
...
params = Juqbox.objparams([N], [Nguard], T, nsteps, Uinit=U0, Utarget=vtarget, Cfreq=om, Rfreq=rot_freq,
                          Hconst=H0, Hsym_ops=Hsym_ops, Hanti_ops=Hanti_ops, wmatScale=0.01, use_cvar=use_cvar)
...
if startFromScratch
  ...
  # cvar has one extra parameter
  if(switch_loss == 4)
    D1 = 12 # Number of B-spline coefficients per frequency, sin/cos and real/imag
    nCoeff = 2*Ncoupled*Nfreq*D1 + 1 # factor '2' is for sin/cos
    pcof0 = (rand(nCoeff) .- 0.5).*maxpar*0.1
    min_t = -1000.0
    max_t = 1000.0
    pcof0[end] = rand(1)[1] # t is randomly initialized between 0 and 1
  end
...
wa = Juqbox.Working_Arrays(params,nCoeff,nquad)
if(switch_loss ==4)
    prob = Juqbox.setup_ipopt_problem(params, wa, nCoeff, vcat(minCoeff, [min_t]), vcat(maxCoeff, [max_t]), maxIter=maxIter, lbfgsMax=lbfgsMax, startFromScratch=startFromScratch, ipTol=ipTol,acceptTol=acceptTol, acceptIter=acceptIter, nodes=nodes, weights=weights, switch_loss=switch_loss, hyper=hyper)
else
    prob = Juqbox.setup_ipopt_problem(params, wa, nCoeff, minCoeff, maxCoeff, maxIter=maxIter, lbfgsMax=lbfgsMax, startFromScratch=startFromScratch, ipTol=ipTol,acceptTol=acceptTol, acceptIter=acceptIter, nodes=nodes, weights=weights, switch_loss=switch_loss, hyper=hyper)
end
println("Initial coefficient vector stored in 'pcof0'")
println("using ", nquad, " quadrature nodes")
println("loss function = ", switch_loss, " (1:RN; 2:RS; 3:RA; 4:cvar), with hyper = ", hyper)
